...
function help() {
    if [ "${1}" == 'long' ]; then
        ...
        echo "  -o <directory>, --outputdirectory=<directory>    The directory in which the clips should be saved."
        echo "  -i <path>, --input=<path>                        The full path to the video from which the clipps should be extracted."
        ...
        echo "  -t <directory>, --tmpdir=<directory>             The temporary directory for adding the keyframes. If not specified, "
        echo "                                                   the video /tmp is used as temporary location. This is only used for "
        echo "                                                   adding keyframes."
        echo "  --nvidia <param>                                 Use the <param> nvidia encoder to do the job."
        echo "                                                   a List of nvidia encoders that can be used is available in the second"
        ...
        echo "  -i, --input=<path>           The full path to the video from which the clipps should be extracted."
        echo "  -k, --addkeyframes           Add keyframes at the timestamps specified with -c, or --clips before"
        echo "                               starting the extraction progress."
        echo "  -t , --tmpdir=<dir>          The temporary directory for adding the keyframes. If not specified, "
        echo "                               the video /tmp is used as temporary location. This is only used for "
        echo "                               adding keyframes."
        echo "  --nvidia=<param>             Use the <param> nvidia encoder to do the job."
        echo "  -q, --quiet                  Be less verbose with the output, although not completely silent."
